...
class Sumrhe:
    def __init__(self, bim_path=None, rhe_path=None, sum_path=None, save_path=None, pheno_path=None, out=None, chisq_threshold=0, nbin=1, \
            log=None, mem=False, allsnp=False, verbose=False, filter_both=False):
        self.mem = mem
        self.log = log
        ...
        self.herits = np.zeros((self.npheno, self.nblks))
    ...
    def _run_jackknife(self, idx):
        ...
        self.hsums[idx][1] = np.sqrt(np.sum(np.square(self.herits[idx] - self.hsums[idx][0]))*(self.nblks-1)/self.nblks)
