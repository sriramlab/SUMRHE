...
These scripts require your own authorized inputs. No research datasets are
distributed here. Paths under `/path/to/` are placeholders; configure input and
output locations outside this checkout before running a script. The simulation
workflow generates phenotypes conditional on supplied genotypes; it does not
establish that those genotypes are synthetic. The historical bundled simulator
