...
geno=/path/to/project/summaryRHE/300k/split_${sample_size}/geno/${sample_size}_split
    for c in sims_0.25_test_0.01 sims_0.25_test_0.1 sims_0.25_test sims_0.4_test_0.01 sims_0.4_test_0.1 sims_0.4_test sims_null_test; do
        sim_folder=/path/to/project/summaryRHE/300k/split_${sample_size}/sims/${c}/

        folder_path=/path/to/project/SUM_RHE/
        ldak=/path/to/project/softwares/ldak5.2.linux


        out=/path/to/scratch/SUM_RHE/outputs/${c}/sumher_gcta/${k}/
        if [[ $sample_size == "50k" ]]; then
            out=/path/to/scratch/SUM_RHE/outputs/50k/${c}/sumher_gcta/${k}/
        fi
