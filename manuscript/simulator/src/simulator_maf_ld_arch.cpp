...
void read_annot (string filename){
    ...
    cout << "Number of SNP Partitions: " << Nbin << endl;
    /*	
        cout<<"jackbin"<<endl;
...
void read_simul_par(std::string filename){
    ...
    while(std::getline(ifs, line)){
        ...
        if (i==0){
            ...
            if (simul_par(5, 0) != h2_vals.sum()) cout << "Annotation heritability and total h2 don't match up!" << endl << h2_vals.sum() << 
                " " << simul_par(5,0);
            cout<<endl;
        }
        i++;
...
void make_maf_ld_effect(double p_casual,double ld_ex,double maf_ex,double min_maf,double max_maf,double h2,int num){
    ...
    MatrixXdr cas;
    cas.resize(Nsnp,Nbin);

    // change how to pick causal SNPs
    bool fix_ncausals = false;
    if (!fix_ncausals){
        for (int i=0;i<Nsnp;i++){
            for (int j=0; j<Nbin; j++){
                cas(i,j)=coin()*annot_bool[i][j];
        ...
        while (ncausals != 0){
            int tmp = get_snp_idx();
            int ischosen = cas.row(0).sum();
            if ((ischosen == 0) && (maf_ld(tmp,2) >= min_maf) && (maf_ld(tmp,2) <= max_maf) ){
                for (int i=0; i<Nsnp; i++){
                    if (annot_bool[tmp][i] ==1)
                        cas(tmp,i) = 1;
                }
                ncausals--;
            }
    ...
    for (int k=0; k < Nbin; k++){
        A_effect=maf_ld.col(0).array()*maf_ld.col(1).array()*cas.col(k).array();
        double cons_fact=(double)h2_vals(k, 0)/A_effect.sum();

        // Fix per-SNP heritability instead of total h2
        //double cons_fact=(double)h2/Nsnp;

        cout << "h2: " << h2_vals(k,0) << endl
